...
class SGDM(object):
    ...
    def backward(self, activation_prev, weights, upstream_grad):
        ...
        self.delta_weights_current = (self.momentum * self.delta_weights_current) +\
                                        ((1- self.momentum) * delta_weights)

        delta_bias = np.sum(upstream_grad, axis=0, keepdims=True)
        if hasattr(self, 'delta_bias_current') == False:
            self.delta_bias_current = np.zeros_like(delta_bias)
        self.delta_bias_current = (self.momentum * self.delta_bias_current) +\
                                        ((1- self.momentum) * delta_bias)
