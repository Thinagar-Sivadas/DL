...
class Sequential(object):
    ...
    def compile(self, loss, inputs, target, batch=8):
        ...
        self.target = target
        self.batch = batch
        self._initialise_param(inputs=self.inputs)

    def _initialise_param(self, inputs):
        ...
        for ind, layer in enumerate(self.layers):
            ...
            if 'layer' in layer.__module__:
                if (layer.weights.size == 0) or (layer.bias.size == 0):
                    print(f'Initialising weights and bias for {layer.name}')
                    layer._init_param(n_input_neurons=input_neurons, next_layer=next_layer)
                elif layer.weights.shape[0] != input_neurons:
                    print(f'Reinitialising weights and bias for {layer.name} due to change in input neurons')
                    layer._init_param(n_input_neurons=input_neurons, next_layer=next_layer)

                input_neurons = layer.n_neurons

    def train(self, epochs=100, plot_cost=True):
        self.cost = []
        ...
        while epoch < epochs+1:
            ...
            cost = float(f'{(self.loss.output + self._regularisation_cost()):.6f}')
    ...
    def _regularisation_cost(self):
        regularizer = 0
        for layer in self.layers:
            if 'layer' in layer.__module__:
                regularizer += layer.optimiser.regulariser.output
        return regularizer
    ...
    def __str__(self):
        ...
        for layer in self.layers:
            ...
            if 'layer' in layer_module[-1]:
                ...
                param.append(layer.weights.size + layer.bias.size)
                total_param += param[-1]
